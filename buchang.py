...
class CameraApp(QWidget, Observable, ThreadSafeObject):
            ...
            if depth_data is not None and self.data:
                for idx, item in enumerate(self.data):
                    ...
                    if z == 0:
                        ...
                        continue
                    # 计算补偿值
                    compensation_z = self.calculate_compensation(x, y,z)
                    print(f"新的(x, y) = ({x}, {y}) 的补偿值: {compensation_z}")
                    z += compensation_z  # 根据补偿值调整z
                    x_mm = (x - cx) * z / fx
                    y_mm = (y - cy) * z / fy
                    ...
                    # 更新坐标
                    new_item = item.copy()
                    z_offset = 100 if class_id in ['boxblue', 'boxgray'] else 25
                    new_item['Coordinate'] = [x_mm, y_mm, z_m + z_offset]
    ...
    def calculate_compensation(self, new_x, new_y, actual_z):
        new_data = np.array([[new_x, new_y]])
        predicted_z_new = self.model.predict(new_data)[0]
        compensation = actual_z - predicted_z_new
        return compensation
